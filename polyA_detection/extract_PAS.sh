...
awk '$NF=="PASS"{print $1}' $inputTSV |
    ...
    perl -ane 'if(($F[1] & 0x10)==0){$l=0;$l+=$1 while($F[5]=~/(\d+)[MDN]/g);$pos=$F[3]+$l-1}else{$pos=$F[3]}; print($F[0],"\t",$F[2],"\t",$pos,"\n")' |
    awk 'BEGIN{print "ReadID\tPolyA_len\tChr\tPAS_positon"}NR==FNR{a[$1]=$9}NR>FNR{print $1"\t"a[$1]"\t"$2"\t"$3}' $inputTSV - > $outputFile
